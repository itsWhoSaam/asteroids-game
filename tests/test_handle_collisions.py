...
def make_game(tmp_path, player, asteroids, shots):
    """A Game wired to the given world, saving into tmp_path."""
    return Game(player, asteroids, shots, save_path=tmp_path / "game_save.json")
...
def test_player_collision_costs_a_life_but_not_the_process(tmp_path):
    ...
    handle_collisions(asteroids, shots, player, game)  # must not raise SystemExit
