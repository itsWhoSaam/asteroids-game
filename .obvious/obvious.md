...
- Smoke checks that work everywhere:
  ...
  - `uv run python .obvious/evidence/proof_drones.py` — drone evidence: turret shots riding the shared pipeline, PNGs to `/tmp/obv-evidence/`.
  - `uv run python .obvious/evidence/proof_shop.py` — shop evidence: purchases and the powerup HUD, PNGs to `/tmp/obv-evidence/`.
  - `uv run python .obvious/evidence/proof_idle.py` — idle-economy evidence: click-damage chip and mint loop, PNGs to `/tmp/obv-evidence/`.
  - `uv run python .obvious/evidence/proof_powerups.py` — bought-powerup evidence: activations and timers, PNGs to `/tmp/obv-evidence/`.
  - `uv run python .obvious/evidence/proof_integration.py` — integration evidence: multi-system frames, PNGs to `/tmp/obv-evidence/`.
  - `uv run python .obvious/evidence/proof_insanity_core.py` — insanity-core evidence: combo meter build-up/cap, hit-stop freeze pair, and dash i-frame blink PNGs to `/tmp/obv-evidence/`.
  - `uv run python .obvious/evidence/proof_threats.py` — threats evidence: boss spawn + health bar + minion checkpoints, saucer crossing + enemy fire, and black-hole pull frames to `/tmp/obv-evidence/`.
  - `uv run python .obvious/evidence/proof_chaos.py` — chaos evidence: mystery `?` drop, curse reveal, and bomb field-clear PNGs to `/tmp/obv-evidence/`.
...
| File | Role |
|---|---|
| `main.py` | Entry point; pygame init, sprite groups, main 60 FPS loop with the hit-stop gate (`effective_frame_dt` freezes the sim while the freeze decays on real dt), collision handling reporting hits to `Game`, click-damage input and the idle destruction-diff/mint poll (idle core), boss/saucer/black-hole scheduling and the per-frame `blackhole.refresh_live_holes` publish, the dash key in the event pump |
...
| `game.py` | `Game` — run state (score, lives, wave, phase); respawn/invulnerability grants, game-over and full-restart resets (engagement F2); the combo meter wiring — `register_kill` pays `points × multiplier`, `break_combo` on dash/life-loss |
| `constants.py` | Tunables: screen 1280x720, player, asteroid, shot parameters; the `PALETTE` table (visual V1); the `INSANITY` block — combo, hit-stop, dash, boss, saucer, black-hole, and mystery tunables plus the six `SFX_*` cues |
...
| `asteroid.py` | `Asteroid` — movement (black-hole pull + chrono scale), `split()` on hit, the `mintable`/`cullable` diff flags, click-chip state; `Boss` — the fifth-wave multi-hit rock with minion checkpoints, chip immunity, and the edge-slide clamp |
| `asteroidfield.py` | `AsteroidField` — spawns asteroids from screen edges on a timer; cadence and speed band come from the pure `wave_params(wave)` (engagement F3), which also flags boss waves |
...
| `shot.py` | `Shot` — player bullets; homing shots steer via a class-level targets ref (the `speed_scale` precedent), pierce rides the sweep |
...
| `hud.py` | `Score` — run score + persistent high score (`game_save.json`), `points_for()` size table, `draw_hud()` overlay, `draw_game_over()` overlay with the run-summary stats block, `WaveBanner` flash (engagement F3); `ComboMeter` (chain, window, milestone chirps), the DASH ready/cooling slot, and `draw_boss_bar` |
