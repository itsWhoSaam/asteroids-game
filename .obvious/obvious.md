...
- Smoke checks that work everywhere:
  ...
  - `uv run python .obvious/evidence/proof_magnet.py` — magnet evidence: a before frame (the green M drop uncollected on the hull, floats at their spawn points, no tag) vs the active-pull frame (the credit float and triple pickup bent toward the ship under the green `MAGNET 7s` tag), to `/tmp/obv-evidence/`, with collection, pull-bend, popup-hold, and distance-closed self-checks asserted.
  - `uv run python .obvious/evidence/proof_boss.py` — boss evidence: the full-HP frame (armor rings in the HP bar's hostile red, the full bar, the mode-scaled drift armed), the hit-flash frame (the warm-white ring riding the hull's outside, the bar drained from the right), and the defeat frame (Score 300, the drop pickup at the death site under the +pts popup, no bar without a live boss), to `/tmp/obv-evidence/`, with ring/flash/HP-bar pixels, the one-diff mint, the exactly-one guaranteed drop, and the boss_spawned/boss_hit/boss_defeated event counts asserted.
...
| `player.py` | `Player` — triangle ship, rotate/move/shoot; the SHIFT dash (impulse with its own decay, i-frames via `max()` on the invulnerability timer, cooldown), curse handling (REVERSE flips control signs), pierce/homing/bomb branches, and the magnet arm (`magnet_timer`, `has_magnet`) |
| `asteroid.py` | `Asteroid` — movement (black-hole pull + chrono scale), `split()` on hit, the `mintable`/`cullable` diff flags, click-chip state with the pure `crack_stage` gate (Tier 2) whose drawn ink web deepens with chip damage and resets on split (children start from fresh `chip_damage`); `Boss` — the fifth-wave multi-hit rock with minion checkpoints, chip immunity, and the edge-slide clamp; the capstone payoff: it mints through the ordinary destruction diff, a landed shot flashes the hull (dt-decayed), the layered armor rings draw in the HP bar's hostile red, the drift scales by the difficulty mode's multiplier, and the sweep's drop site guarantees one chaos-table pickup on defeat |
...
| `challenge.py` | `challenge` — the daily seeded challenge (Tier 3): pure `daily_seed` (UTC YYYYMMDD) and `daily_slug` (ISO date), plus the defensive `load_daily_best`/`record_daily_score` pair over one `daily_best` dict key through hud's read-modify-write merge; the spawn RNG itself hangs off `asteroidfield.spawn_rng`, stamped by `restart_run` at both launch hooks |
| `powerups.py` | `PowerUp` pickups — seven buffs (SHIELD/RAPID/TRIPLE/PIERCE/HOMING/BOMB/MAGNET), two mystery-only curses, and the violet `?` wildcard; pure `drops_powerup`/`drop_type`/`pick_type`/`mystery_pick_type` rolls and `magnet_pull` — the pure attraction step (force toward the ship, distance-eased, speed-capped, no teleport); effect data lives in `constants.py` tables (engagement F4, Tier 3, insanity chaos) |
...
| `sound.py` | Procedural SFX — stdlib `array`/`math` envelopes in `pygame.mixer.Sound`, built at startup; `play()`/`play_explosion()` degrade to a silent no-op on any mixer failure; mute state set via `set_muted` (engagement F6); the ambient music loop (`build_ambient_loop`, `update_music` per-frame gameplay gate) rides the same mute/volume seams and degrade contract |
