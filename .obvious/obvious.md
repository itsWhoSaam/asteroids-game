...
- Smoke checks that work everywhere:
  ...
  - `uv run python .obvious/evidence/proof_ufo.py` — UFO-saucer evidence: the saucer mid-crossing after its 45 s cadence entry, and the aimed shot in flight toward the ship, to `/tmp/obv-evidence/`, with entry-edge, inward-heading, from_ufo-tag, aim, and shoot-down-mint self-checks asserted.
...
| `asteroid.py` | `Asteroid` — movement, `split()` on hit; click-chip state with the pure `crack_stage` gate (Tier 2) whose drawn ink web deepens with chip damage and resets on split (children start from fresh `chip_damage`) |
| `asteroidfield.py` | `AsteroidField` — spawns asteroids from screen edges on a timer; cadence and speed band come from the pure `wave_params(wave, mode)` (engagement F3), scaled by the difficulty mode's multipliers (Tier 2) |
| `powerups.py` | `PowerUp` pickups — drifting SHIELD/RAPID/TRIPLE/MAGNET drops; pure `drops_powerup`/`pick_type` rolls over the four-way pool; `magnet_pull` — the pure attraction step (force toward the ship, distance-eased, speed-capped, no teleport); effect data lives in `constants.py` tables (engagement F4, Tier 3) |
...
| `shot.py` | `Shot` — player bullets; `from_drone` tags turret shots riding the same pipeline (run-stats PR); `from_ufo` tags saucer shots the same way (Tier 3) |
| `ufo.py` | `UFO` saucer + `UFOSpawner` — dt spawn cadence (~45 s), off-screen edge entry, perpendicular sinusoidal drift, aimed fire with the muzzle outside the hull, headless-safe inked draw, `destroyed_ufos` frame diff for the mint poll (culls never pay) (Tier 3) |
