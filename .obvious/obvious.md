...
| `game.py` | `Game` — run state (score, lives, wave, phase, pause flag); respawn/invulnerability grants, game-over and full-restart resets (engagement F2); owns the run-stats instance and resets it in place on restart (run-stats PR); the combo meter wiring — `register_kill` pays `points × multiplier`, `break_combo` on dash/life-loss |
